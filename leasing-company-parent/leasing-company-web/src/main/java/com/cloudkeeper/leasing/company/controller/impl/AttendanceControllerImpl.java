...
public class AttendanceControllerImpl implements AttendanceController {
    ...
    public Result<Page<AttendanceVO>> page(@ApiParam(value = "出勤查询条件", required = true) @RequestBody AttendanceSearchable attendanceSearchable, @ApiParam(value = "排序条件", required = true) Pageable pageable) {
        BooleanBuilder booleanBuilder = attendanceService.defaultBooleanBuilder(attendanceSearchable);
        Page<Attendance> attendancePage = attendanceService.findAll(booleanBuilder, pageable);
        List<AttendanceVO> attendanceVOS = attendancePage.getContent().stream().map(Attendance::convert).collect(Collectors.toList());
        Page<AttendanceVO> attendanceVOPage = new PageImpl<>(attendanceVOS, pageable, attendancePage.getTotalElements());
