...
public interface PrincipalController {
    ...
    @GetMapping("/to/{id}")
    PrincipalTO findOneTO(@ApiParam(value = "用户id", required = true) @PathVariable String id);

    @ApiOperation(value = "查询用户集合", notes = "查询用户集合")
    @PostMapping("/to/ids")
    List<PrincipalTO> findAllTO(@ApiParam(value = "用户id集合", required = true) @RequestBody List<String> ids);
}
